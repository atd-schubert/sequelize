...
const QueryGenerator = {
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    const values = {
      table: this.quoteTable(this.extractTableDetails(tableName, this.options)),
      attributes: attrStr.join(', '),
      comments: _.template(comments, this._templateSettings)({ table: this.quoteTable(this.extractTableDetails(tableName, this.options)) })
    };
    ...
    for (const attributeName in attributes) {
      ...
      if (definition.indexOf('NOT NULL') > 0) {
        attrSql += _.template(query, this._templateSettings)({
          tableName: this.quoteTable(this.extractTableDetails(tableName, this.options)),
